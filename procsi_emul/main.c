...
typedef enum
{
   ...
   //ERR,     /*!< Erreur. */ conflit avec NCURSES
	...
	{   switch(c)
		...
		case KEY_UP:
			...
			clrtoeol();
                        ...
                        choix = malloc(strlen(item_name(current_item(my_menu))) * sizeof(char)+2);
...
void display_execution(int num_instruction, mot * tab_mot_instruction, int nb_instruction, int* registres,int nb_reg, int PC, int SP, int SR){
	...
	register_items = (ITEM **)calloc(nb_reg + 1 +3, sizeof(ITEM *)); //+3 pour PC SP et SR
        ...
        operateur = (char *) malloc (8* sizeof(char));
        mode_string = (char *) malloc (7* sizeof(char));
	...
	for(i = 0; i < nb_instruction; ++i){
            ...
            if(is_brut == 0){
                ...
                strcat(tab_instruction[i], mode_string);
                strcat(tab_instruction[i], "    "); 
